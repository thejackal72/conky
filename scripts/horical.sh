...
while [ $b -gt 0 ]; do
    case "$y" in
    1) TOPLINE="$TOPLINE Lu";;
    2) TOPLINE="$TOPLINE Ma";;
    3) TOPLINE="$TOPLINE Me";;
    4) TOPLINE="$TOPLINE Gi";;
    5) TOPLINE="$TOPLINE Ve";;
    6) TOPLINE="$TOPLINE Sa";;
    7) TOPLINE="$TOPLINE Do";;
    esac
    b=$[$b-1]
    y=$[$y+1]
    if [ $y -eq 8 ]
...
echo '${goto 220}''${font mono:bold:size=8}'$TOPLINE | sed 's/Do/${color red}Do${color}/g' | sed 's/Sa/${color red}Sa${color}/g'
echo '${goto 220}''${font mono:bold:size=8}''${color 1793d1}'$OVER '${color 5BED1B}'$TODAY'${color}'$REST
